...
function extract(markdownStr, markdownFileName, skeletonFilename, srcLang, trgLang, options) {
    ...
    parseMarkdown(markdownStr.replace(/\\/g, '\\\\'));
    ...
    function onText(text, inline) {
        const punctuation = '[;\.!\?]';

        text.split(new RegExp(`(${punctuation})\\s`)).forEach((text, id, arr) => {
            if (new RegExp(`^${punctuation}$`).test(arr[id - 1])) {
    ...
    function addUnit(skl, xliff) {
        ...
        skeleton = skeleton.slice(0, currentSkeletonPosition) + skeleton.slice(currentSkeletonPosition).replace(skl, function (str, offset) {
    ...
    function onCode(code, lang) {
        ...
        if (lang === 'js' || lang === 'javascript') {
            try {
                comments = extractComments.fromString(code) || extractComments.fromString('(' + code + ')');
        ...
        if (tag.state === 'open') {
            ...
            if (attributes.some(isTranslationAttr) || isBlock[tag.type]) {
                ...
                attributes.forEach(key => {
                    isTranslationAttr(key) && addUnit(tag.attrs[key]);
                });
            } else {
                temp[tag.type] = ++i;
                let str = '';
